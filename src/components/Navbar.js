...
const Navbar = () => {
  const { signOutSite, user } = useContext(AuthContext);
  const handelSignOut = ()=>{
    signOutSite ()
      .then(()=>{
        toast.error('Logn Out ')
      })
        ...
        <nav className='md:ml-auto flex flex-wrap items-center text-base justify-center'>
          ...
          <>
            { 
              ...
              <button onClick={handelSignOut} className='inline-flex items-center bg-gray-300 border-0 py-1 px-3 focus:outline-none hover:bg-gray-200 rounded text-base mt-4 md:mt-0'>
                Logout
                <svg
                  fill='none'
                  stroke='currentColor'
